"""
Professor KJ - AI Note Taker
You skip classes, I make notes.
...
def check_password():
    ...
    <div class="password-screen">
        ...
        <h2>Professor KJ</h2>
        <p class="subtitle">You skip classes, I make notes.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("Unlock", use_container_width=True, type="primary"):
        if pwd == APP_PASSWORD:
            ...
            st.error("Wrong password. Are you sure you're a Professor KJ student?")

    st.markdown("""
    <div style="text-align: center; margin-top: 2rem;">
        <span style="font-size: 0.75rem; color: #334155;">Hint: Ask Professor KJ</span>
    </div>
    """, unsafe_allow_html=True)
    return False
...
<div class="hero">
    ...
    <h1>Professor KJ</h1>
    <p class="tagline">You skip classes, I make notes.</p>
...
if 'last_notes' in st.session_state:
    ...
    with col_a:
        ...
        st.download_button("📄 Notes (Word)", data=docx_bytes,
            file_name=f"Professor KJ_{safe_name}_{datetime.now(IST).strftime('%Y%m%d')}.docx",
    ...
    with col_c:
        md_content = f"# Lecture Notes: {subject_saved or filename}\n*Generated by Professor KJ: {datetime.now(IST).strftime('%B %d, %Y')}*\n\n{notes}"
        st.download_button("📝 Markdown", data=md_content,
            file_name=f"Professor KJ_{safe_name}_{datetime.now(IST).strftime('%Y%m%d')}.md",
...
if 'viewing_note_id' in st.session_state:
    ...
    if view_data:
        ...
        with col_a:
            ...
            st.download_button("📄 Notes (Word)", data=docx_bytes,
                file_name=f"Professor KJ_{safe_name}_{eid}.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        ...
        with col_c:
            ...
            st.download_button("📝 Markdown", data=md_content,
                file_name=f"Professor KJ_{safe_name}_{eid}.md",
...
<div class="app-footer">
    <div class="footer-brand">🎓 Built by KJ for Professor KJ</div>
